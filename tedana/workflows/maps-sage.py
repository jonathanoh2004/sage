...
def t2smap_workflow(
    ...
    if fitmode == "all":
        (t2s_limited, s0_limited, t2s_full, s0_full) = decay.fit_decay_sage(
            catd, tes, fittype
        )
    else:
        (t2s_limited, s0_limited, t2s_full, s0_full) = decay.fit_decay_ts_sage(
            catd, tes, fittype
        )
    ...
    OCcatd = combine.make_optcom(catd, tes, t2s=t2s_full, combmode=combmode) # REMOVED MASKSUM PARAMETER
